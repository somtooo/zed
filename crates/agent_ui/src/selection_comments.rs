...
pub fn selection_comment(
    ...
    let anchor_range: Range<Anchor> = newest_selection.start..newest_selection.end;
    ...
    let callback: SelectionCommentCallback = Rc::new(
        move |comment_text: String, window: &mut Window, cx: &mut App| {
            ...
            // Per-call clone: this callback is `Fn` (reusable), so the owned
            // `comment_text` cannot be moved into the deferred closure below.
