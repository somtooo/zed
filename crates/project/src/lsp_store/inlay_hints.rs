...
impl BufferInlayHints {
    ...
    // TODO kb how did we do hover & click then??? what's below that????
...
impl LspStore {
    pub(super) fn resolve_inlay_hint(
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client() {
            ...
            cx.spawn(async move |_, cx| {
                ...
                // TODO kb why not wait in the proto conversion where everyone else does this?
                buffer
                    .update(cx, |buffer, _| {
    ...
    pub(super) async fn handle_resolve_inlay_hint(
        ...
        // TODO kb what?? we are loosing the edits for version mismatch????
        if response_hint
            .text_edits
            .as_ref()
